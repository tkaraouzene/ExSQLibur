...
sub ADD {
	...
	if (&check_patient_header($header)) {
		...
		chomp;
		my ($patient_id,$sex,$f1,$f2,$comment,$patho,$seq_platform,$seq_model,$seq_place,$seq_date) = split /\t/;
		my $is_runs_ace = 0;
		...
		&my_select($dbh,
			   {table => $config->{table_name}->{patient},
			    ...
			    values => [$patient_id],
			    verbose => $config->{verbose}
			   }) and warnq warn_mess."$patient_id is already present in the db, skipping it" and next; 
		...
		&insert_values($dbh,
			       {table => $config->{table_name}->{patient},
				fields => ["id","sex","reads_file1","reads_file2","is_runs_ace","comment","pathology","exome"],
				values => [$patient_id,$sex,$f1,$f2,$is_runs_ace,$comment,$patho,$exome_id],
				verbose => $config->{verbose}});
	    }
...
sub check_patient_header {
    ...
    if ($header eq "id\tsex\treads_file1\treads_file2\tcomments\tpathology\tseq_plateforme\tseq_model\tseq_place\tseq_date") {
