...
# $rp,$vp,$rm,$vm: the number of Ref/Variant reads on the Plus/Minus strand;
...
#
# We calculate $covRat = ($rp + $rm + $vp + $vm) / $totalCov, and use
...
sub callGeno {
    my ($status,$value) = ("0","0") ;
    if (@_ != 6) {
	warn "ERROR: callGeno needs 6 args: totCov,R+,V+,R-,V-,MinCov\n" ;
	warn "Got ", scalar(@_), " args: @_\n" ;
	return($status,$value) ;
    }
    my ($genotype_parametresR,$totalCov,$rp,$vp,$rm,$vm) = @_ ;

    my $minCov = @$genotype_parametresR[0];
    if (! $minCov > 0) {
	warn "ERROR: callGeno called with minCov $minCov, but it must be a positive int\n" ;
	return($status,$value) ;
    }
    if (! $totalCov > 0) {
	warn "ERROR: callGeno called with totCov $totalCov, but it must be a positive int\n" ;
	return($status,$value) ;
    }
    ...
    my $covRat = ($rp + $rm + $vp + $vm) / $totalCov ;
	...
	my $call1 = &callGenoStrand($rp,$vp,$genotype_parametresR) ;
	my $call2 = &callGenoStrand($rm,$vm,$genotype_parametresR) ;
